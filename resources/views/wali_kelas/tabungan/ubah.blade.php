        ...
        <div class="panel panel-default card-view">
            <div class="panel-wrapper collapse in">
                <div class="panel-body">
                    <form data-toggle="validator" role="form" action="{{ route('wali.tabungan.ubah', $tbg->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <select name="siswa_id" class="form-control select2" required>
                                    ...
                                    @foreach ($siswa as $sw)
                                    <option @if($tbg->tabungan->siswa->id == $sw->id) selected  @endif value="{{ $sw->id }}">{{ $sw->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label class="control-label mb-10">Jumlah Tabungan</label>
                                <input type="number" class="form-control" name="jml_tabungan"
                                    placeholder="Masukkan Jumlah Tabungan" required
                                    value="{{ $tbg->jumlah }}">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <a href="{{ route('wali.tabungan') }}" type="button" class="btn btn-danger">Kembali</a>
